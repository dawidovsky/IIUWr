...
(eval '((lambda-rec (append l1 l2)
   ...
   (cons 1 (cons 2 null))(cons 3 (cons 4 null))))

;; map

(eval '((lambda-rec (map f l)
                    (if (null? l)
                        null
...
(eval '(let (app (lambda-rec (append l1 l2)
                                (if (null? l1)
                                    l2
                                    (cons (car l1)(append (cdr l1)l2)))))
           ((lambda-rec (reverse l1)
                        (if (null? l1)
                            l1
                            (app (reverse (cdr l1))(cons (car l1) null))))
...
(eval '((lambda x (+ (car (cdr x)) (car (cdr (cdr x))))) 1 2 3))

;; leniwe listy
#|
(eval '(let (lcons (lambda (x f) (cons x f)))
               (let (lhead (lambda (l) (car l)))
                 (let (ltail (lambda (l) ((cdr l))))
                   (let (nats-from
                         (lambda-rec (nats-from m)
                                     (lcons
                                      m
                                      (lambda ()(nats-from (+ m 1))))))
                     (let (nats (nats-from 0))
                       (let (take (lambda-rec (take n l)
                                              (cond [(null? l) null]
                                                    [(= n 0) null]
                                                    [true (cons (lhead l)
                                                                (take (- n 1) (ltail l)))])))
