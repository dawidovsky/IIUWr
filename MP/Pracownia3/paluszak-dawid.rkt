...
;; Roszerzony algorytm Euklidesa okazuje się być rozwiązaniem
;; Idea użycia go wzięta z repetytoriów i wskazówek Pana Biernackiego

(define (solve-ax+by=1 a b)
  (define (helper a b x y)
    (if (= b 0)
        (cons y x)
        (helper b (modulo a b) y (- x (* (floor (/ a b)) y)))))
  (helper a b 1 0))
...
(define (RSA-convert-list intlist key)
  (let ((n (key-modulus key))) ;;n = (car key)
...
(define (encrypt-and-sign message recipient-public-key sender-private-key)
  ...
  (make-signed-message x (RSA-transform (compress x) sender-private-key))))

(define (authenticate-and-decrypt cyphertext recipient-private-key sender-public-key)
  (let ([x (RSA-decrypt (message cyphertext) recipient-private-key)])
    (if (= (RSA-transform (signature cyphertext) sender-public-key)
           (compress (message cyphertext)))
        x
        (display "Problem - podpis sie nie zgadza"))))
